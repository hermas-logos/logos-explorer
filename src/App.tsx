...
type TabType = 'summary' | 'slides' | 'mindmap' | 'quiz' | 'sources';
...
export default function App() {
  ...
  // Markdown Loading for Study Guides
  ...
  useEffect(() => {
    if (activeEpisode) {
      ...
      const mdPath = activeProfile === 'academic' 
        ? `/content/ep04/academic-guide.md` 
        : `/content/ep04/esl-guide.md`;
      ...
      fetch(mdPath)
        ...
        .catch(() => {
          setMarkdownContent(`### Course Material Loaded\nStudy guides are available online. Open your PWA directories to ensure \`public/content/ep04/\` assets are successfully copied.`);
          setLoadingMarkdown(false);
        });
    }
  }, [selectedEpisodeId, activeProfile, activeEpisode]);
              ...
              {filteredEpisodes.map((ep) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-auto pt-3 border-t border-dashed border-stone-800/60">
                    <span className="text-[10px] uppercase font-bold text-stone-500 px-2 py-0.5 rounded border border-stone-800">INSPIRING QUOTATIONS</span>
                    {ep.hasMindMap && <span className="text-[10px] uppercase font-bold text-stone-500 px-2 py-0.5 rounded border border-stone-800">MIND MAP</span>}
                    {ep.hasSlides && <span className="text-[10px] uppercase font-bold text-stone-500 px-2 py-0.5 rounded border border-stone-800">SLIDE CAROUSEL</span>}
                    {ep.hasQuiz && <span className="text-[10px] uppercase font-bold text-stone-500 px-2 py-0.5 rounded border border-stone-800">ACTIVE QUIZ</span>}
                  </div>
        ...
        {activeView === 'hub' && selectedEpisodeId && activeEpisode && (
          <div className="flex flex-col gap-6 animate-fadeIn">
            ...
            <div className="flex items-center justify-between border-b border-stone-800/40 pb-4">
              ...
              <span className="text-xs text-stone-500 font-medium">
                Published: {activeEpisode.publishedDate}
              </span>
            </div>
            ...
            <div className="flex border-b border-stone-800/60 overflow-x-auto gap-2 scrollbar-none pb-1">
              ...
              {activeEpisode.hasSlides && (
                <button
                  onClick={() => setActiveTab('slides')}
                  className={`px-4 py-2.5 border-b-2 text-xs uppercase tracking-widest font-bold whitespace-nowrap transition-all \${activeTab === 'slides' ? 'border-amber-500 text-amber-500 bg-amber-500/5' : 'border-transparent text-stone-400 hover:text-amber-400'}`}
                >
                  <Sliders className="w-3.5 h-3.5 inline mr-1.5" /> Slide Deck
                </button>
              )}
              {activeEpisode.hasMindMap && (
                <button
                ...
                >
                  <Map className="w-3.5 h-3.5 inline mr-1.5" /> Mind Map
                </button>
              )}
              {activeEpisode.hasQuiz && (
                <button
            ...
            <div className="min-h-[300px] animate-fadeIn">
              ...
              {activeTab === 'summary' && (
                <div className={`prose max-w-none text-sm leading-relaxed p-6 border rounded-xl \${darkMode ? 'prose-invert bg-slate-900/20 border-stone-800 text-stone-300' : 'bg-white border-stone-200 text-stone-800'}`}>
                  {loadingMarkdown ? (
                    <div className="py-12 text-center text-stone-500 animate-pulse">Loading study materials...</div>
                  ) : (
                        ...
                        if (paragraph.startsWith('*   ')) {
                          return (
                            <ul key={pIdx} className="list-disc list-inside pl-4 space-y-1">
                              {paragraph.split('\n').map((li, liIdx) => (
                                <li key={liIdx} className="text-stone-300">{li.replace('*   ', '').replace('-   ', '')}</li>
                              ))}
                            </ul>
                          );
                        }
              ...
              {/* SLIDES TAB */}
              {activeTab === 'slides' && (
                <div className="border border-stone-800/80 rounded-xl overflow-hidden shadow-lg">
                  <SlideCarousel />
                </div>
              )}
